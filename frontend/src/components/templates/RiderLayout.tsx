...
export function RiderLayout() {
  ...
  // Auto-login demo rider if not authenticated or wrong role (for demo purposes)
  // Wait for hydration to complete before setting user to avoid race conditions
  useEffect(() => {
    if (_hasHydrated && (!user || user.role !== 'rider')) {
      setUser(demoUsers.rider);
    }
  ...
  const navLinks = [
    { to: '/rider/home', label: 'Book Ride', icon: MapPin },
    { to: '/rider/history', label: 'History', icon: Clock },
    { to: '/rider/payments', label: 'Payments', icon: CreditCard },
    { to: '/rider/support', label: 'Support', icon: HelpCircle },
  ];
              ...
              <Link to="/rider">
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-xs text-white/70 font-medium">Rider</p>
                  </div>
                </motion.div>
              </Link>
                ...
                <nav className="flex items-center gap-2">
                  {navLinks.map((link) => {
                    ...
                    const isActive = location.pathname === link.to ||
                      (link.to === '/rider/home' && location.pathname === '/rider');
                    return (
                      <Link
                        key={link.to}
                        to={link.to}
  ...
  return (
    <div
      className={cn(
        ...
        isConnected ? 'bg-success-500/20 text-success-100' : 'bg-error-500/20 text-error-100'
      )}
    >
      {isConnected ? (
        <>
